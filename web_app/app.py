...
def predict_landmarks():
    """
    Lightning-fast prediction using raw landmarks sent from client.
    Skips image encoding/decoding and server-side MediaPipe.
    Expects: { "landmarks": [{"x": 0.5, "y": 0.5}, ...] }
    """
    if not classifier:
    ...
    try:
        ...
        if not data or 'landmarks' not in data:
            return jsonify({'prediction': None, 'error': 'Missing landmarks'}), 200
            
        landmarks = data['landmarks']
        # Handle cases where multiple hands are sent or nested list
        if isinstance(landmarks, list) and len(landmarks) > 0:
            # If it's a list of hands, take the first one
            first_hand = landmarks[0] if isinstance(landmarks[0], list) else landmarks
            label = classifier.predict_from_landmarks(first_hand)
            return jsonify({'prediction': label, 'landmarks': [first_hand]})
            
        return jsonify({'prediction': None}), 200
    except Exception as e:
        return jsonify({'prediction': None, 'error': str(e)}), 200
